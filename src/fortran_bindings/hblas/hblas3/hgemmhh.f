      Subroutine HGEMMHH(TRANSA, TRANSB, M, N, K, ALPHA, A, LDA,
     $  B, LDB, BETA, C, LDC)
C
      LOGICAL LSAME
      EXTERNAL LSAME
C
     ...
     $          BZERO, BONE
      ...
      Complex*16    A(2,LDA,*), B(2,LDB,*), C(2,LDC,*), ALPHA(2),
     $  BETA(2)
C
...
C
            ...
            ELSE
              HTMP2S    = 
     $          ALPHA(1)*HTMP1S - ALPHA(2)*CONJG(HTMP1J) +
     $          BETA(1)*C(1,I,J) - BETA(2)*CONJG(C(2,I,J))
              C(2,I,J) = 
     $          ALPHA(1)*HTMP1J + ALPHA(2)*CONJG(HTMP1S) + 
